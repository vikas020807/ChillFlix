...
class ChillflixController extends Controller
{
    /**
     * @param  mixed  $genreId
     * @return mixed
    ...
    {
        // dd(Http::withToken(config('services.tmdb.token'))
        ...
        $trending = Cache::remember('movies_trending', 60 * 60, function () {
            return Http::withToken(config('services.tmdb.token'))
                ->get('http://api.themoviedb.org/3/trending/movie/day')
                ->json()['results'];
        });

        $chillflixgenres = Cache::remember('movies_genres', 60 * 60, function () {
            return Http::withToken(config('services.tmdb.token'))
            ->get('http://api.themoviedb.org/3/genre/movie/list')
            ->json()['genres'];
        });
        ...
        $genres = collect($chillflixgenres)->mapWithKeys(function ($genre) {  /** @phpstan-ignore-line */
